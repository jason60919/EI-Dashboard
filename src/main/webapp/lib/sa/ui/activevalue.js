...
(function ($) {
    //define plugin name
    var pluginName = 'activevalue';

    //create plugin class
    function Plugin(element, options) {

        this.logger = log4jq.getLogger({
            loggerName: 'activevalue.js'
        });
    ...
    Plugin.prototype = {
        init: function () {
            ...
            var activeRender = '<div class="widget-activevalue-number"><span></span></div><div class="widget-activevalue-unit"><span class="widget-activevalue-unit-text" lang="en" langtag="Uni_003" ></span><span class="widget-activevalue-unit-val">' + unit + '</span></div>';
            plugin.$el.append(activeRender);
            var $unit = plugin.$el.find('.widget-activevalue-unit-val');
            
            if(unit === ''){
                var $unit = plugin.$el.find('.widget-activevalue-unit');
                $unit.hide();
            }
            plugin.update(opts.value);
            $unit.html(opts.unit);
        },
        update: function (newValue) {
            var plugin = this;
            plugin.logger.info('update:' + newValue);
            var $number = plugin.$el.find('.widget-activevalue-number span');
